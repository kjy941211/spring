...
public class JDBCTest {
    ...
    // 여기에 여러 설정 잘 되어있는지 함수 만들어서 테스트 여러개 할 예정
    // 테스트 단위마다 함수 하나 만들어주면 됨
    // 함수 단위마다 테스트 해준다고해서 단위테스트 (Unit test)
    // JUnit 라이브러리 사용 예정

    // 모든 단위테스트에서 사용할 드라이버를 설정하는 부분이 필요함

    // 아래 모든 함수 호출전에 무조건 한번 이거 실행해줘라고 할 때
    // @BeforAll --> 시작할 때 한 번 실행함
    ...
    public static void setup() {
        ...
        try {
            ...
            e.printStackTrace(); // 상세한 에러정보를 프린트 할 수 있음
        }
    }
    ...
    public void testConnection() {
        ...
        try(Connection con = DriverManager.getConnection(url, "scoula", "1234")) {
            ...
            fail(e.getMessage());
            // ==> junit에게 테스트 하려고 했던 것 실패했다라고 명시적으로 알려주는 역할의 함수
        }
    }
}
